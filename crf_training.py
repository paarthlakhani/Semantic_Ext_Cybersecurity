...
class CRFTraining:
    ...
    def word2features_train(self, i):
        ...
        word = self.train_table_word_attrs[i][0]
        pos_tag = self.train_table_word_attrs[i][1]
        ...
        if i != total_length - 1:
            if i >= 1:
                if self.train_table_word_attrs[i - 1][0] != '0.0':
                    word_before = self.train_table_word_attrs[i - 1][0]
                    features['unigram_before'] = word_before
            if i >= 2:
                if self.train_table_word_attrs[i - 1][0] != '0.0' and self.train_table_word_attrs[i - 2][0] != '0.0':
                    word_before_1 = self.train_table_word_attrs[i - 1][0]
                    word_before_2 = self.train_table_word_attrs[i - 2][0]
                    bigram_before = word_before_1 + " " + word_before_2
                    features['bigram_before'] = bigram_before
            if i <= total_length - 2:
                if self.train_table_word_attrs[i + 1][0] != '0.0':
                    word_after = self.train_table_word_attrs[i + 1][0]
                    features['unigram_after'] = word_after
            if i <= total_length - 3:
                if self.train_table_word_attrs[i + 1][0] != '0.0' and self.train_table_word_attrs[i + 2][0] != '0.0':
                    word_after_1 = self.train_table_word_attrs[i + 1][0]
                    word_after_2 = self.train_table_word_attrs[i + 2][0]
    ...
    def word2features_test(self, i):
        ...
        word = self.test_table_word_attrs[i][0]
        pos_tag = self.test_table_word_attrs[i][1]
        # token_label = self.word_attribute_table[i][2]
        ...
        if i != total_length - 1:
            if i >= 1:
                if self.test_table_word_attrs[i - 1][0] != '0.0':
                    word_before = self.test_table_word_attrs[i - 1][0]
                    features['unigram_before'] = word_before
            if i >= 2:
                if self.test_table_word_attrs[i - 1][0] != '0.0' and self.test_table_word_attrs[i - 2][0] != '0.0':
                    word_before_1 = self.test_table_word_attrs[i - 1][0]
                    word_before_2 = self.test_table_word_attrs[i - 2][0]
                    bigram_before = word_before_1 + " " + word_before_2
                    features['bigram_before'] = bigram_before
            if i <= total_length - 2:
                if self.test_table_word_attrs[i + 1][0] != '0.0':
                    word_after = self.test_table_word_attrs[i + 1][0]
                    features['unigram_after'] = word_after
            if i <= total_length - 3:
                if self.test_table_word_attrs[i + 1][0] != '0.0' and self.test_table_word_attrs[i + 2][0] != '0.0':
                    word_after_1 = self.test_table_word_attrs[i + 1][0]
                    word_after_2 = self.test_table_word_attrs[i + 2][0]
                    bigram_after = word_after_1 + " " + word_after_2
                    features['bigram_after'] = bigram_after
    ...
    def create_x_train(self):
        ...
        for i in range(self.train_table_word_attrs.shape[0]):
            if self.train_table_word_attrs[i][2] != '0.0':
    ...
    def create_y_label_for_test(self):
        ...
        for i, word_attr in enumerate(self.test_table_word_attrs):
            if word_attr[2] != '0.0':
                y_label_test.append(word_attr[2])
        return y_label_test
